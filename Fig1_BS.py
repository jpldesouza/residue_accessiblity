...
# Manual panel layout (figure coordinates)
posA = [0.1, 0.55, 0.38, 0.38]
posB = [0.6, 0.55, 0.38, 0.38]
posC = [0.1, 0.1, 0.38, 0.38]
posD = [0.6, 0.1, 0.38, 0.38]

axA = fig.add_axes(posA)
axB = fig.add_axes(posB)
...
# (a) external PNG
# ============================================================
axA.imshow(plt.imread("Fig1a.png"))
axA.set_axis_off()
panel_label(axA, "(a)")
...
# (b) external PNG
# ============================================================
axB.imshow(plt.imread("Fig1b.png"))
axB.set_axis_off()
panel_label(axB, "(b)")
...
axC.set_xlim(1/N, 1)
axC.set_xticks([1/N,1])
axC.set_xticklabels([r"$1$", r"$N$"])
axC.set_ylim(min(y), max(y))
...
axD.fill_between(phi_right_vec, 0, 1.2*(1/chi_vec), color='#CCCCFF', alpha=1)
axD.fill_between(phi_left_vec,  0, 1.2*(1/chi_vec), color='#CCCCFF', alpha=1)

axD.fill_between(phi_right_vec, 0, 1/chi_vec, color='#6666FF', alpha=1)
axD.fill_between(phi_left_vec,  0, 1/chi_vec, color='#6666FF', alpha=1)
...
axD.text(0.36, 0.88, "two phases",fontsize=16,rotation=-36)
axD.text(0.5, 1.1, "single phase",fontsize=16,rotation=-45)

panel_label(axD, "(d)")

plt.savefig("Fig1_BS2.png", dpi=300)
plt.show()
